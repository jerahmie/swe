...
//
pub fn gaussian2d(sigma: f64, x0: f64, y0: f64, dx: f64, dy: f64, nx: usize, ny: usize) -> Array2<f32>{
    let mut gaussian2d = Array2::<f32>::zeros((nx,ny));
    for ((i,j), elt) in gaussian2d.indexed_iter_mut(){
       *elt = exp(-1.0*((((i as f64)*dx -x0).powf(2.0))/(2.0*sigma.powf(2.0)) +
               (((j as f64)*dy -y0).powf(2.0))/(2.0*sigma.powf(2.0)))) as f32;
    }
    gaussian2d
}
